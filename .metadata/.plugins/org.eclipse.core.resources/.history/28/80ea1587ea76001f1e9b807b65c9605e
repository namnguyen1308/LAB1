...
void displaySecond7SEG(int num) {
	...
	HAL_GPIO_WritePin(LED11_GPIO_Port, LED11_Pin, (segment_data[num] & 0x01) ? SET : RESET);  // LED1
	HAL_GPIO_WritePin(LED22_GPIO_Port, LED22_Pin, (segment_data[num] & 0x02) ? SET : RESET);  // LED2
	HAL_GPIO_WritePin(LED33_GPIO_Port, LED33_Pin, (segment_data[num] & 0x04) ? SET : RESET);  // LED3
	HAL_GPIO_WritePin(LED44_GPIO_Port, LED44_Pin, (segment_data[num] & 0x08) ? SET : RESET);  // LED4
	HAL_GPIO_WritePin(LED55_GPIO_Port, LED55_Pin, (segment_data[num] & 0x10) ? SET : RESET);  // LED5
	HAL_GPIO_WritePin(LED66_GPIO_Port, LED66_Pin, (segment_data[num] & 0x20) ? SET : RESET);  // LED6
	HAL_GPIO_WritePin(LED77_GPIO_Port, LED77_Pin, (segment_data[num] & 0x40) ? SET : RESET);  // LED7
}
void trafficLightControl() {
    ...
    for (int i = 5; i > 3; i--) {
        ...
        displaySecond7SEG(i - 3);    // Hiển thị số đếm ngược cho tuyến đường 2 (2 giây đèn xanh)
